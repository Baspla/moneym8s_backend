...
class User(db.Model):
    ...
    guard_id = mapped_column(String(64), unique=True, index=True)
...
class Membership(db.Model):
    ...
    group_id = mapped_column(Integer, ForeignKey('groups.id', ondelete='CASCADE', onupdate='CASCADE'))
...
class Transaction(db.Model):
    ...
    id = mapped_column(Integer, primary_key=True)
    group_id = mapped_column(ForeignKey('groups.id', ondelete='CASCADE', onupdate='CASCADE'))
    description = mapped_column(String(128))
    amount = mapped_column(Float)
    issuer_id = mapped_column(Integer, ForeignKey('memberships.id', ondelete='SET NULL', onupdate='CASCADE'))
    paid_by_id = mapped_column(Integer, ForeignKey('memberships.id', ondelete='SET NULL', onupdate='CASCADE'))
    created_at = mapped_column(DateTime, default=db.func.now())
...
class TransactionSplit(db.Model):
    ...
    id = mapped_column(Integer, primary_key=True)
    transaction_id = mapped_column(Integer, ForeignKey('transactions.id', ondelete='CASCADE', onupdate='CASCADE'))
    member_id = mapped_column(Integer, ForeignKey('memberships.id',ondelete='SET NULL', onupdate='CASCADE'))
    amount = mapped_column(Float)
    created_at = mapped_column(DateTime, default=db.func.now())
...
class LogEntry(db.Model):
    ...
    membership_id = mapped_column(Integer, ForeignKey('memberships.id',ondelete='CASCADE', onupdate='CASCADE'))
    ...
    transaction_split_id = mapped_column(Integer, ForeignKey('transaction_splits.id',ondelete='CASCADE', onupdate='CASCADE'))
